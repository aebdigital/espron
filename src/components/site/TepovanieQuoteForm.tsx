...
export default function TepovanieQuoteForm() {
  ...
  async function handleSubmit() {
    ...
    setError(null);
    const lines = [
      `Typ nábytku: ${data.typNabytku}`,
      `Úroveň zašpinenia: ${data.urovenZaspinenia}`,
      `Plocha / počet kusov: ${data.plocha}`,
    ...
    const result = await submitContact({
      ...
      email: data.email,
      phone: data.telefon,
      subject: "Cenová ponuka – tepovanie",
      message: lines.join("\n"),
      turnstileToken,
    });
    setSending(false);
  ...
  return (
    <div className="relative rounded-[2rem] border border-border bg-white p-8 shadow-[0_12px_40px_rgba(15,29,74,0.07)] md:p-10">
      ...
      {step === 0 && (
        <div>
          <h3 className="mb-6 text-xl font-bold text-foreground">
            Informácie o type nábytku
          </h3>
          <p className="mb-4 text-sm text-foreground/60">
            Uveďte či ide o kreslo, gauč alebo koberec?
          </p>
          <div className="grid gap-3 sm:grid-cols-2">
            {["Gauč / sedačka", "Kreslo", "Koberec", "Matrac", "Sedačka v aute"].map((t) => (
              <button
                key={t}
                type="button"
                onClick={() => { set("typNabytku", t); next(); }}
      ...
      {step === 1 && (
        <div>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {[
              { label: "Nízka", desc: "Bežné používanie, pár škvŕn" },
              { label: "Stredná", desc: "Viditeľné škvrny, staršie nečistoty" },
              { label: "Vysoká", desc: "Silné znečistenie, zvieratá, deti" },
            ].map((t) => (
              <button
                key={t.label}
                type="button"
                onClick={() => set("urovenZaspinenia", t.label)}
                className={`rounded-2xl border px-5 py-4 text-left transition-all ${
                  data.urovenZaspinenia === t.label
                    ? "border-primary bg-primary/5 text-primary"
                    : "border-border hover:border-primary/40"
                }`}
              >
                <p className="text-sm font-bold">{t.label}</p>
                <p className="mt-0.5 text-[11px] text-foreground/50">{t.desc}</p>
              </button>
            ))}
          </div>
        </div>
      ...
      {step === 2 && (
        <div>
          <h3 className="mb-6 text-xl font-bold text-foreground">
            Plocha alebo počet kusov nábytku
          </h3>
          <p className="mb-4 text-sm text-foreground/60">
            Plocha v m² (pri kobercoch) alebo počet miest na sedenie pri viac miestnych sedačkách.
          </p>
          <input
            type="text"
            placeholder="napr. 15 m² koberec  /  3-miestna sedačka  /  2 kreslá"
            value={data.plocha}
          ...
          <p className="mt-3 text-xs text-foreground/40">
            Orientačná hodnota stačí — presné nacenenie doladíme po obhliadke fotiek.
          </p>
        </div>
      )}
      ...
      {step === 3 && (
        <div>
          ...
          <p className="mb-4 text-sm text-foreground/60">
            V ktorom meste sa bude vykonávať tepovanie?
          </p>
          <input
            type="text"
      ...
      {step === 6 && (
        <div>
          ...
          <div className="divide-y divide-border rounded-2xl border border-border">
            {[
              { label: "Typ nábytku", value: data.typNabytku || "—" },
              { label: "Zašpinenie", value: data.urovenZaspinenia || "—" },
              { label: "Plocha / počet", value: data.plocha || "—" },
