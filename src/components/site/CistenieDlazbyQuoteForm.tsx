...
export default function CistenieDlazbyQuoteForm() {
  ...
  async function handleSubmit() {
    ...
    setError(null);
    const lines = [
      `Typ dlažby: ${data.typDlazby}`,
      `Stav dlažby: ${data.stavDlazby}`,
      `Plocha dlažby: ${data.plocha} m²`,
    ...
    <div className="relative rounded-[2rem] border border-border bg-white p-8 shadow-[0_12px_40px_rgba(15,29,74,0.07)] md:p-10">
      ...
      {step === 0 && (
        <div>
          ...
          <p className="mb-4 text-sm text-foreground/60">
            O aký typ plochy ide?
          </p>
          <div className="grid gap-3 sm:grid-cols-2">
            {["Zámková dlažba", "Terasa", "Chodník", "Parkovisko / Príjazdová cesta"].map((t) => (
              <button
                key={t}
                type="button"
                onClick={() => { set("typDlazby", t); next(); }}
      ...
      {step === 1 && (
        <div>
          ...
          <p className="mb-4 text-sm text-foreground/60">
            Aký typ znečistenia chcete odstrániť?
          </p>
          <div className="grid gap-3 sm:grid-cols-2">
            {["Machy a plesne", "Oleje a mastné škvrny", "Bežné znečistenie", "Kombinácia"].map((t) => (
              <button
                key={t}
                type="button"
                onClick={() => set("stavDlazby", t)}
                className={`rounded-2xl border px-5 py-4 text-left text-sm font-medium transition-all ${
                  data.stavDlazby === t
                    ? "border-primary bg-primary/5 text-primary"
      ...
      {step === 3 && (
        <div>
          ...
          <input
            type="text"
            placeholder="napr. Prešov, Poprad, Spišská Nová Ves…"
            value={data.lokalita}
      ...
      {step === 4 && (
        <div>
          ...
          <p className="mb-4 text-sm text-foreground/60">
            Pripojte niekoľko fotografií dlažby, aby sme mohli prispôsobiť cenovú ponuku aktuálnemu stavu.
          </p>
      ...
      {step === 6 && (
        <div>
          ...
          <div className="divide-y divide-border rounded-2xl border border-border">
            {[
              { label: "Typ dlažby", value: data.typDlazby || "—" },
              { label: "Stav dlažby", value: data.stavDlazby || "—" },
              { label: "Plocha dlažby", value: data.plocha ? `${data.plocha} m²` : "—" },
